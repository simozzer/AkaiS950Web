...
var AkaiAudio = (function () {
  ...
  var CAL = {
     ...
     *
     * Every calibration take up to run 17 was recorded with 20-30% of its samples pinned
     * within 0.1 dB of -0.49 dBFS. The crossfade is the one TWO-TONE measurement in the rig,
     * and that is what made it the worst casualty: a weak tone sharing a signal with a
     * limited strong one is dragged down by the strong one's gain reduction, so the error
     * grows as the two tones become unequal - which is towards the edges of the overlap.
     * Against the clean re-record the old readings were out by 0.2 dB at x = 0.1, 1.5 dB at
     * x = 0.5 and 6.1 dB at x = 0.9, and they gave a curve of cos(pi x / 2) ^ 1.44 with the
     * pair dipping 1.3 dB at the midpoint. "No standard crossfade does that" was written in
     * these notes at run 15 and treated as a curiosity. It was the limiter.
     *
     * Clean, the pair conserves power to 0.1 dB and the fade is close to a plain equal-power
     * cosine through the first half, then steeper than one beyond it.
     *
     ...
     *
     * WIDTH 9 DISAGREES WITH THE OTHERS AT ITS LAST KEY, and it survived the re-record: at
     * x = 0.9 it reads -17.6 where width 13 at 0.929 and width 21 at 0.909 both read -21.6.
     * Four decibels, against the 0.1 the rest agree to. With the gain quantised this is what
     * you would expect - two (i, N) pairs landing on nearby x can still fall on different
     * integer steps - so the real rule is arithmetic on i and N rather than a function of x,
     ...
     *
     * It is not the table read at some x: it does not move with the key at all. 17 pairs in
